...
class CameraSweepAS(object):
    ...
    def action_server_launcher(self, goal):
        ...
        while flag==1:
            ...
            ref_posy = self.robot_odom.posy
            laser_data_min= self.robot_scan.min_distance
            #not stopping on obstacle turn based on angle
            if laser_data_min < goal.approach_distance:
                if self.feedback.feedback.current_distance_travelled<10:
                    if self.robot_scan.closest_object_position<0:
                        self.robot_controller.set_move_cmd(0.0,0.8)
                    else:
                        self.robot_controller.set_move_cmd(-0.09,0.5)
                else:
                    if self.robot_scan.closest_object_position<0:
                        self.robot_controller.set_move_cmd(0.0,-0.8)
                    else:
